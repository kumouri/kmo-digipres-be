...
| F.4 ContractPdfService + ContractTemplateService + ContractService (explicit-boolean SOW-from-quote) | done | 8951f7b | compileJava + compileTestJava → BUILD SUCCESSFUL exit 0 (full suite skipped per §6 — CI at PR) | switchIfEmpty grep over service/contract/: 3 functional hits — ContractTemplateService:48 genuine 3705 not-found; ContractService:101 genuine 3707 not-found; ContractService:241 genuine 3705 active-template not-found. NO switchIfEmpty(create/spawn). All other grep hits are comments/Javadoc. PASS. | none — F-D10 deviation (ContractNumberIndexInitializer pattern) is from F.3 and applies unchanged. F.4 has no deviations. |
