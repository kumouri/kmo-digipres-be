...
| Q3 | `RepairVsReplaceReasoner` (+ financing flag) + `QuoteBookingService` (booking-link SMS, no live Cal.com; optional Documenso SOW on REPLACE) | DONE | (Q3 commit) | `RepairVsReplaceReasonerTest` 7/0 (flips old→REPLACE+financing / young→REPAIR / unknown→DIAGNOSTIC; rationale always non-blank); `QuoteAcceptIT` 5/0 (accept→1 booking SMS+ACCEPTED; re-accept→0 second SMS; DECLINED→4436; missing→4435; no-phone→transitions, no SMS). Documenso SOW on REPLACE = documented seam, NOT wired live (deferred). |
| Q4 | public intake endpoint(s) + accept + office quote-inbox (list/detail) + token issuer + finish demo seed + openapi regen + CLAUDE.md T8 | DONE | (Q4 commit) | `QuoteIntakeService` orchestrator + 4 controllers + 2 DTOs; `QuoteIntakeIT` 7/0, `QuoteInboxIT` 5/0, `QuoteNowModuleGateIT` 2/0. Regression green: `AiVisionServiceIT` 5/0, `EquipmentVisionIT` 6/0, `module.homeservices.*` 85/0 (29 classes), `OpenApiEndpointIT` 2/0. openapi.json: NO quoting routes (module OFF in OpenApiEndpointIT → FE hand-writes all quoting api/*.ts, the T1-T7 precedent; reverted the non-deterministic churn). Token = reused PublicWidgetTokenService (no sibling needed — empty-diff). |
