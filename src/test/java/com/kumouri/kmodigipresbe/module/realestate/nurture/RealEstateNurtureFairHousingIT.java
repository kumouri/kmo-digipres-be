...
/**
 * T1 — the HEADLINE IT: the Fair-Housing guardrail on every RE nurture outbound message. With the
 * realestate + nurture modules both on, {@code RealEstateNurtureAutoConfiguration} wires
 * {@code FairHousingCopyFilter} onto the shared {@code NurtureMessageComposer}, so the default-OFF
 * {@code NurtureRunner} (enabled in-test) screens every send.
 *
 * <p>Proves:
 * <ul>
 *   <li>a clean RE template is sent verbatim;</li>
 *   <li>a campaign step with a <strong>non-compliant template</strong> ("perfect for families") → the SMS
 *       actually sent is the vetted <strong>safe fallback</strong> (zero non-compliant text);</li>
 *   <li>{@code aiPersonalize=true} + WireMock Anthropic returns a <strong>non-compliant rewrite</strong>
 *       ("safe neighborhood, great for kids") → the lint catches the AI draft → the safe fallback is sent;</li>
 *   <li>an opted-out contact → zero send (the engine's TCPA skip, unchanged).</li>
 * </ul>
 *
 * <p>Mirrors {@code NurtureRunnerIT} (default-OFF runner enabled in-test, {@code @MockitoBean}
 * TwilioSmsService, deterministic {@code runDueOnce()} block, scheduled tick pushed far out, Anthropic →
 * WireMock via {@code @DynamicPropertySource}) + adds {@code kmosf.modules.realestate.enabled=true} so the
 * filter is wired. §7: no live send anywhere — the runner is default-OFF in prod/CI; this IT opts in AND
 * mocks the send seam.
 */
@SpringBootTest
@Import({TestcontainersConfiguration.class, RealEstateNurtureFairHousingIT.FixedClockConfig.class})
@TestPropertySource(properties = {
        ...
        "kmosf.mail.smtp.username=nurture@gateway.test",
...
class RealEstateNurtureFairHousingIT {

    private static final Instant NOW = Instant.parse("2026-06-09T12:00:00Z");
    private static final String PHONE = "+12145550123";
    private static final String SAFE_SMS = "Hi Pat, checking in from Gateway Realty — reply YES to chat.";
    private static final String ANTHROPIC_API_KEY = "sk-ant-test-re-nurture-fake";
    ...
    void seed() {
        ...
        mongo.save(Tenant.builder()
                .id(tenantId).slug("re-nurture-fh-it-" + tenantId)
                .displayName("Gateway Realty FH IT").status(Tenant.TenantStatus.ACTIVE)
    ...
    /** One SMS step 0, with the given template + aiPersonalize flag (the RE A-bucket sweep-all segment). */
    ...
    void cleanTemplate_isSentVerbatim() {
        NurtureCampaign campaign = seedCampaign(
                "Hi {firstName}, the market's moved — want a quick value update?", false);
        seedEnrollment(campaign.getId(), seedContact("Pat", null));

        runner.runDueOnce().block();

        assertThat(smsBodies).singleElement()
                .isEqualTo("Hi Pat, the market's moved — want a quick value update?");
    }

    @Test
    void nonCompliantTemplate_isReplacedWithSafeFallback() {
        // A campaign template carrying a Fair-Housing steering phrase — must NOT be sent.
        NurtureCampaign campaign = seedCampaign(
                "Hi {firstName}, this home is perfect for families in a safe neighborhood!", false);
        seedEnrollment(campaign.getId(), seedContact("Pat", null));

        runner.runDueOnce().block();

        // The lint caught it → the vetted safe fallback was sent; zero non-compliant text.
        assertThat(smsBodies).singleElement().isEqualTo(SAFE_SMS);
        assertThat(smsBodies.get(0).toLowerCase()).doesNotContain("perfect for families");
        assertThat(smsBodies.get(0).toLowerCase()).doesNotContain("safe neighborhood");
    ...
    @Test
    void nonCompliantAiRewrite_isCaughtByLint_safeFallbackSent() {
        // The template is clean, but the AI rewrite injects steering language — the lint must catch the
        // AI draft (proves the screen covers the AI-personalized path, not just the template).
        stubAnthropic("Great for kids and perfect for families in a safe neighborhood!");
        ...
        NurtureCampaign campaign = seedCampaign(
                "Hi {firstName}, want a quick no-pressure update on your home's value?", true);
        seedEnrollment(campaign.getId(), seedContact("Pat", null));

        runner.runDueOnce().block();

        assertThat(smsBodies).singleElement().isEqualTo(SAFE_SMS);
        assertThat(smsBodies.get(0).toLowerCase()).doesNotContain("great for kids");
        assertThat(smsBodies.get(0).toLowerCase()).doesNotContain("perfect for families");
    }

    @Test
    void optedOutContact_isSkipped_zeroSend() {
        NurtureCampaign campaign = seedCampaign("Hi {firstName}, perfect for families!", false);
