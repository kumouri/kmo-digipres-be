package com.kumouri.kmodigipresbe.module.styleconsult.support;
...
/**
 * T9 — in-memory {@link FileStorageService} stub for the StyleConsult ITs — exercises the server-side
 * {@code putBytes} inspiration-photo store without a real object store. A faithful clone of the T8
 * {@code QuotingItStorageTestConfig}: a real {@code @Bean @Primary} (NOT {@code @MockBean}) so it stays
 * off the functional-shard-splinter list — the precedented Attachment/file-IT storage stub, not a
 * behaviour mock.
 *
...
public class StyleConsultItStorageTestConfig {

    @Bean
    @Primary
    FileStorageService styleConsultItFileStorageService() {
