...
/**
 * Tech Copilot (T13) — the <strong>AI-08 grounding min-score gate</strong>. The in-memory
 * {@link VectorIndex} returns a fixed similarity score of {@code 0.9} for every stored chunk, so the gate is
 * deterministically testable:
 * <ol>
 *   <li><strong>gate ABOVE the hit score (0.95 &gt; 0.9)</strong> → every retrieved chunk is dropped → the
 *       no-chunks short-circuit fires → HANDOFF and the model is NEVER called (a low-relevance retrieval can
 *       no longer ground a confident, mis-cited answer).</li>
 *   <li><strong>gate BELOW the hit score (0.5 &le; 0.9)</strong> → the chunk clears the gate → the grounded
 *       path runs exactly as before (the gate is a floor, not a behavior change above it).</li>
 * </ol>
 * The default ({@code min-score} absent → 0.0 = off) is exercised by the sibling T13 ITs, which stay green
 * — this class is the opt-in proof.
 */
@SpringBootTest
@Import({TestcontainersConfiguration.class, TechCopilotMinScoreIT.InMemoryVectorIndexConfig.class})
@TestPropertySource(properties = {
        ...
        // The fixed in-memory hit score is 0.9; gate at 0.95 so it falls BELOW the floor → handoff.
        "kmosf.techcopilot.min-score=0.95",
        "kmosf.techcopilot.handoff-message=I don't have that documented in the manuals on file. "
                + "Escalate to a senior tech."
})
class TechCopilotMinScoreIT {

    private static final String ANTHROPIC_API_KEY = "sk-ant-test-t13-ms-fake";
    ...
    @Test
    void chunkBelowMinScore_isDropped_handsOff_withoutCallingTheModel() {
        // A doc exists and IS retrieved (fixed score 0.9), but the 0.95 gate drops it → nothing to ground on.
        create("Furnace Manual", EquipmentType.FURNACE,
                "Fault E3 rollout reset: power off, press the reset button until it clicks.");
        stubGroundedAnswer("(must NEVER be sent — the only chunk is below the min-score gate)");

        TechCopilotService.CopilotAnswer ans = copilot.ask("fault E3 rollout reset procedure", null)
                .contextWrite(TenantContextHolder.write(ctx())).block();

        assertThat(ans.handoff()).as("below-min-score chunk → HANDOFF").isTrue();
        assertThat(ans.citations()).isEmpty();
        assertThat(ans.answer().toLowerCase()).contains("don't have that documented");

        // The model was NEVER called — the gate collapsed to the no-chunks short-circuit before the call.
    ...
    private void seedTenant(UUID tid) {
        mongo.save(Tenant.builder()
                .id(tid).slug("t13ms-" + tid)
                .displayName("Summit Mechanical T13 MinScore IT").status(Tenant.TenantStatus.ACTIVE)
    ...
    // ── deterministic in-memory VectorIndex (every stored chunk scores a fixed 0.9) ─────
