...
/**
 * ChairFill CF-2 — drafts a short, on-brand, <strong>Claude-personalized</strong> appointment
 * reminder / confirmation SMS for an upcoming salon booking. A <strong>new additive service that
 * mirrors {@link com.kumouri.kmodigipresbe.integration.gbp.GbpReplyDraftService} /
 ...
 *
 * <p>It does <strong>NOT</strong> modify the reused AI core (the {@code AnthropicAiAssistService}
 * stays empty-diff); this is a sibling caller, the same posture as {@code GbpReplyDraftService} and
 * {@code VoicemailExtractionService}. The system prompt asks for a warm, concise SMS in the owner's
 * brand voice that names the stylist + the last service the client had, never invents facts, and
 * outputs ONLY the SMS body (no markdown / signature / placeholders). The prompt is configurable via
 * {@code kmosf.chairfill.reminder-system-prompt} (blank = the built-in default).
 *
 * <p><strong>Error codes are reused, not re-allocated:</strong> the budget gate owns
 * {@code 1200/1201}, the Anthropic upstream non-200 is {@code 1202}, missing-key is {@code 1203}
 * (CF-2 reuses the AI band unchanged). The {@code RiskTieredPreventionService} caller wraps this
 * <em>best-effort</em> ({@code onErrorResume}): a budget-exhausted {@code 1200}, an upstream
 * {@code 1202}, or a missing-key {@code 1203} all degrade to a generic template reminder — the
 * personalization is a nicety, never a blocker (plan §4 AI-budget mitigation). A blank/whitespace
 * model answer surfaces {@code 1202} too (so the caller falls back rather than texting an empty body).
 */
@Slf4j
public class ReminderCopyService {
    ...
    private static final String DEFAULT_SYSTEM_PROMPT =
            "You write a single short SMS reminding a salon client about their upcoming appointment, "
            + "on behalf of the salon owner. Write in a warm, friendly, concise voice — like a real "
            + "stylist who knows the client personally, not a corporate brand. Greet the client by "
            + "first name when one is given, mention the stylist they're booked with and the service "
            + "they last came in for when those are given, and keep it to ONE or TWO short sentences "
            + "that fit comfortably in a text message. If asked to confirm, ask them to reply to "
            + "confirm the appointment. Do NOT invent facts, prices, times, discounts, names, or "
            + "details not provided. Do NOT include placeholders, links, signatures, emoji, or "
            + "markdown — output ONLY the SMS text itself, nothing else.";
    ...
    /**
     * The personalization inputs the drafter weaves into the reminder. All optional/nullable — the
     * prompt instructs the model to skip anything not provided rather than invent it.
     *
     * @param clientFirstName the client's first name (greeting), or null
     * @param stylistName     the booked stylist's display name, or null
     * @param lastService     the service the client most recently had / is booked for, or null
     * @param appointmentWhen a human phrase for the appointment time (e.g. "Friday at 6:00 PM"), or null
     * @param brandTone       the owner's brand-voice hint (e.g. "warm and a little playful"), or null
    ...
    /**
     * Drafts the reminder SMS. Mirrors {@code GbpReplyDraftService.draftReply}: tenant key → budget
     ...
     */
    public Mono<String> draftReminder(ReminderContext context) {
    ...
    private Mono<CompletionResult> postMessages(String apiKey, ReminderContext context) {
        ...
        return http.post()
                ...
                .onStatus(status -> !status.is2xxSuccessful(), response -> response.bodyToMono(String.class)
                        ...
                        .flatMap(errBody -> Mono.error(new DigiPresBeException(
                                "Anthropic reminder-draft call failed: "
    ...
    private Mono<CompletionResult> parseAnthropicResponse(JsonNode node) {
        ...
        if (text.isBlank()) {
            // No usable reminder text — surface as an upstream failure so the caller's best-effort
            // onErrorResume falls back to the generic template (never texts a blank body).
            return Mono.error(new DigiPresBeException(
                    "Anthropic reminder-draft returned an empty reminder", 1202, 502));
        }
    ...
    private static String buildUserPrompt(ReminderContext c) {
        StringBuilder sb = new StringBuilder();
        sb.append(c.confirm()
                ? "Write an appointment CONFIRMATION reminder text (ask the client to reply to confirm).\n\n"
                : "Write a friendly appointment reminder text.\n\n");
        sb.append("Client first name: ")
                .append(blankToUnknown(c.clientFirstName())).append('\n');
        sb.append("Stylist: ")
                .append(blankToUnknown(c.stylistName())).append('\n');
        sb.append("Service they last had / are booked for: ")
                .append(blankToUnknown(c.lastService())).append('\n');
