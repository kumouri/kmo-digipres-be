...
**OpenAPI:** Spec served at `GET /api/v1/v3/api-docs` (JSON) and `/api/v1/openapi` (Swagger UI), unauthenticated. The live spec is exported by `OpenApiEndpointIT` during the Testcontainers **test** phase to `build/openapi/openapi.json` (the only place a Mongo-backed app boots in CI; the springdoc gradle plugin's `generateOpenApiDocs` forks a full app boot and is NOT on the `check`/build path — it is optional, manual, local-only, and needs a reachable DB). `./gradlew verifyOpenApi` (`check`-gated, runs after `test`) is **bootstrap-tolerant**: while `docs/api/openapi.json` carries `"x-kmosf-spec-status": "bootstrap-placeholder"` the gate never fails — on a green CI run it rewrites the file from the exported spec (commit it; CI also uploads it as the `openapi-spec` artifact). Once a real spec is committed (sentinel gone) the gate enforces drift **strictly** (fail on mismatch); in an env with no generated spec (no Docker) it skips with a loud warning rather than false-failing. **The committed spec is currently the bootstrap placeholder** pending the first green CI run.
